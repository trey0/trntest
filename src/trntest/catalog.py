"""PDS Geosciences Node Orbital Data Explorer (ODE) REST API client -- discovers real LROC WAC
EDR/CDR products by time range without downloading each product's individual label. Low-level,
takes plain scalars/config the same way `cache.py` does. Lists of products are `pandas.DataFrame`s
rather than a list of dataclasses -- simpler grouping/filtering downstream (see `dataset.py`) and
free, readable table display in the notebook.
"""
...
def _required_field(element: ET.Element, tag: str) -> str:
    """`element.findtext(tag)`, raising ValueError (rather than returning None) if missing -- so a
    product missing/malformed one of the expected fields is skipped by parse_catalog_entries'
    surrounding try/except, instead of silently producing a None/garbage row."""
...
def parse_catalog_entries(xml_text: str) -> pd.DataFrame:
    """Parse an ODE `results=opmf` product-query XML response into a DataFrame (columns:
    CATALOG_COLUMNS). Extracts volume/subdir/doy/product from each product's `LabelURL` field
    (matching this package's existing archive path convention exactly), not the awkward
    backslash-separated `RelativePathtoVol` field."""
...
def list_products(
    ...
    """Query ODE for all products of `product_type` observed in [min_time, max_time], paginating as
    needed."""
    frames = []
    offset = 0
    while True:
...
def find_matching_cdr(edr_row: pd.Series, config: TrntestConfig) -> pd.Series | None:
    """Find the CDR counterpart of an EDR catalog row (same orbit, same acquisition), by querying a
    tight time window around the EDR's own start/stop time. Returns None (caller should skip +
    warn, not crash) if no match is found."""
