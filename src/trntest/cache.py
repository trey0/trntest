...
# on a cache hit -- see the early return below) costs nothing in the normal, mostly-cached case and
# only slows down exactly the bulk-fresh-fetch case that needs slowing down.
#
# Raised from 0.2 to 0.5 after a 429 (1hr ban) on the LROC EDR host during a ~200-candidate
# orbit-sequence window -- root cause wasn't fully pinned down (the failure hit on the very first
# request of that run, suggesting the host may already have been degraded rather than this
# session's own pacing being the sole cause), but a more conservative floor is cheap insurance:
# deliberately a single, general constant (not host-specific), since usage is either warm-cache
# (free either way) or a large batch, where the difference is an insignificant fraction of total
# time.
_REQUEST_PACING_SECONDS = 0.5
_MAX_FETCH_ATTEMPTS = 3
# A `Retry-After` longer than this isn't worth retrying inline -- e.g. the 3600s (1hr) CloudFront
# sent LROC's own EDR host during the incident described above. Failing fast lets the caller decide
# whether to abort rather than have `cached_get` silently block for up to an hour.
_MAX_RETRY_AFTER_SECONDS = 30.0

# One shared connection pool (keep-alive) for every fetch this process makes, rather than a fresh
# TCP/TLS handshake per `requests.get` call -- lighter on the remote server and faster for us.
...
def lunaserv_rel_path(layer: str, bbox, width: int, height: int, fmt: str) -> str:
    """Cache path for one Lunaserv `GetMap` response, keyed by (layer, bbox, width, height, format)."""
...
def fetch_lunaserv_getmap(
    ...
    """Fetch and cache one Lunaserv WMS `GetMap` response."""
    params = {
        "request": "GetMap",
        "service": "WMS",
        "version": "1.1.1",
...
def fetch_wac_emp_tile(product_id: str, cache_root: Path, base_url: str) -> Path:
    """Fetch one WAC_EMP PDS4 archive tile (see `ortho_wac_emp.wac_emp_tile_ids_for_bbox`) and cache it
    locally, once."""
...
def lroc_rel_path(dataset: str, volume: str, subdir: str, doy: str, product: str, ext: str) -> str:
    """Cache path mirroring the LROC PDS archive's own volume/subdir/doy layout."""
...
def fetch_lroc_file(
    ...
    """Fetch and cache one LROC EDR/CDR product file (image or label) from the PDS archive."""
