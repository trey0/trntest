...
def dataset_midpoint_datetime(dataset: TrnTestDataSet) -> datetime:
    ...
    start = pd.to_datetime(dataset.images[start_col]).min()
    stop = pd.to_datetime(dataset.images[stop_col]).max()
...
def _ground_track_lonlat(dataset: TrnTestDataSet) -> list[tuple[float, float]]:
    ...
    """
    start_col, stop_col = dataset.time_span_columns
    start_et = illumination.utc_to_et(pd.to_datetime(dataset.images[start_col]).min().to_pydatetime())
    stop_et = illumination.utc_to_et(pd.to_datetime(dataset.images[stop_col]).max().to_pydatetime())
