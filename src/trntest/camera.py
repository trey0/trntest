...
_LIGHTWEIGHT_BORESIGHT_CORRECTION = np.array(
    [
        ...
        [-2.02236824e-04, 9.99997460e-01, 2.24459000e-03],
...
def build_camera(
    ...
    *,
    ...
    cached_crop = isis_wac.cached_crop_path(config)
    if cached_crop.exists():
        # This product's crop was already generated by a prior run -- reuse it directly instead of
        # calling `run_pipeline` (which would force a full ISIS reprocessing pass just to answer one
        # ground-point query the crop can answer just as well; see `isis_wac.ensure_crop_for_camera`'s
        # own module-level rationale). The crop window is centered on this exact pixel by
        # construction (`crop_window_for_frame`), so it's never near the crop's own edges -- no
        # `campt` edge-convergence risk (see `tie_points.crop_footprint_corners_for_camera`'s own
        # comment on that limitation, which only bites near the crop's margins).
        window = isis_wac.crop_window_for_frame(center_frame_index, crop_info["n_frames_for_square_crop"])
        local_line = center_line - window.row_off + 1  # ISIS LINE is 1-based; row_off is 0-based
        target_lon, target_lat = isis_campt.ground_point_at_pixel(cached_crop, isis_wac.SAMPLES / 2.0, local_line)
    else:
        stitched = isis_wac.run_pipeline(k == _REVERSED_TIME_K, frame_timing, config)
        target_lon, target_lat = isis_campt.ground_point_at_pixel(
            stitched.cub_path, isis_wac.SAMPLES / 2.0, center_line
        )
...
def lightweight_footprint_lonlat_deg(
    ...
    comparisons need -- deliberately not using `entry.camera` (which would force a full ISIS
    pipeline run for any not-yet-populated entry) so the map never needs branching logic on
    population state, and is never at risk of silently omitting/misrepresenting entries that
