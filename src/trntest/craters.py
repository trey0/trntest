...
# indexes/filters by each crater's own center *point* (see `_convert_to_geopackage`), not its ellipse
...
def _csv_member_name(zf: zipfile.ZipFile) -> str:
    """The bundle's one crater-data CSV member name.

    :param zf: The opened Robbins zip.
    :returns: The member name.
    :raises ValueError: If the zip doesn't contain exactly one match.
    """
    # A plain `*.csv` glob isn't enough -- `collection_data_inventory.csv` is bundle metadata, not
    # crater rows -- so this also requires `/data/` in the path and excludes anything with
    # "inventory" in the name; exactly one file in the download matches both (see
    # docs/data-sources/robbins-craters.md).
...
def geopackage_path(zip_path: Path) -> Path:
    """The converted GeoPackage's path for `zip_path`.

    :param zip_path: The raw Robbins zip path.
    :returns: `zip_path`, same basename, `.gpkg` suffix.
    """
    return zip_path.with_suffix(".gpkg")


def _convert_to_geopackage(zip_path: Path, gpkg_path: Path, moon_radius_m: float) -> None:
    """One-time conversion from the raw zip to a spatially-indexed GeoPackage (see
    `ensure_geopackage`).

    :param zip_path: The raw Robbins zip path.
    :param gpkg_path: Output GeoPackage path.
    :param moon_radius_m: Sphere radius for the database's CRS -- must match its own declared radius
        (`MOON_RADIUS_M`, 1,737,400m, see docs/data-sources/robbins-craters.md).
    """
    # Reads the CSV directly out of the zip (no unzip-to-disk step needed), builds Point geometry
    # from each crater's own ellipse-fit center (`LAT_ELLI_IMG`/`LON_ELLI_IMG` -- the same center
    # `crater_overlay_layer`'s ellipse construction itself uses, not the separate circle-fit center,
    # so indexing and drawing stay consistent), and writes a GeoPackage. GDAL builds a spatial index
    # (`rtree`) for GeoPackage output by default -- that index is what makes
    # `query_craters_in_bbox`'s `geopandas.read_file(..., bbox=...)` a fast index seek instead of a
    # full-file scan; the raw CSV has no spatial index at all, so this step is the one that actually
    # matters at ~1.3M rows.
    #
    # 38 of 1,296,796 rows have NaN ellipse-fit columns -- dropped, since a crater with no fitted
    # ellipse can't be drawn as one anyway.
...
def ensure_geopackage(config: TrntestConfig | None = None) -> Path:
    """Fetch the raw zip and convert it to a spatially-indexed GeoPackage, caching both.

    :param config: Project config; `load_config()` if not given.
    :returns: The GeoPackage path.
    """
    # Idempotent, like every other fetch helper in this project -- `gpkg_path.exists()`
    # short-circuits the conversion cost.
...
def query_craters_in_bbox(bbox_lonlat_deg: tuple, config: TrntestConfig | None = None) -> geopandas.GeoDataFrame:
    """Query craters whose center falls inside `bbox_lonlat_deg`.

    :param bbox_lonlat_deg: `(minlon, minlat, maxlon, maxlat)` degrees. Not padded internally -- pad
        before calling if padding is wanted (see `crater_overlay_layer`'s own
        `_QUERY_BBOX_PADDING_FRACTION`).
    :param config: Project config; `load_config()` if not given.
    ...
    # pixel-exact depending on driver internals, so both stages matter.
    config = config or load_config()
...
def _ellipse_polygon(
    ...
    """An ellipse polygon in local isotropic-meters CRS.

    :param center_x_m: Ellipse center, local meters.
    :param center_y_m: Ellipse center, local meters.
    :param major_km: Major axis, full length (`DIAM_ELLI_MAJOR_IMG`'s own "diameter" naming, matching
        `DIAM_CIRC_IMG`'s convention -- see docs/data-sources/robbins-craters.md).
    :param minor_km: Minor axis, full length, same convention as `major_km`.
    :param angle_deg: Rotation, degrees (see the body comment for the convention this assumes).
    :returns: The ellipse polygon.
    """
    # Exact, no projection distortion: the reprojection into this CRS already happened on the center
    # point before this is called (see `crater_overlay_layer`), not on a pre-built ellipse shape.
    #
...
def raster_bbox_deg(raster_path) -> tuple:
    """`raster_path`'s footprint, projected into the crater database's geographic CRS.
...
def query_craters_for_raster(
    ...
    """`query_craters_in_bbox`, but the AOI comes from `raster_path`'s own georeferencing
    (`raster_bbox_deg`) rather than a bbox the caller derives by hand.
...
def crater_overlay_layer(
    ...
    **layer_kwargs,
    ...
    """Build a `plotting.OverlayLayer` of Robbins crater ellipses covering `raster_path`'s own
    footprint, in `raster_path`'s own CRS -- the entry point notebook cells call.

    :param raster_path: A raster to overlay craters onto.
    :param config: Project config; `load_config()` if not given.
    :param min_major_km: Drop craters whose ellipse-fit major axis (`DIAM_ELLI_MAJOR_IMG`, a full
        axis length/diameter, not a radius -- see `_ellipse_polygon`'s own docstring) is smaller than
        this, if given.
    :param min_arc_img: Drop craters whose `ARC_IMG` (the fraction, 0-1, of the crater's own rim
        circumference that was actually traceable and used in its ellipse fit) is below this, if
        given. See the body comment for why this is only meaningful combined with `min_major_km`.
    :param layer_kwargs: Passed through to `plotting.OverlayLayer`.
    :returns: The overlay layer, ready to pass into `plotting.plot_overlay`/`plot_overlay_toggle`'s
        `layers=[...]`, or `None` if no craters fall in view (e.g. a very small footprint, or
        everything filtered out) -- callers should build their `layers` list conditionally
        (`layers=[l for l in (craters.crater_overlay_layer(...),) if l is not None]` or similar).
    """
    # A typical camera footprint here (tens to ~100km across) contains thousands of sub-km craters --
    # the full, unfiltered database at that scale visually overwhelms the base image rather than
    # reading as an annotation, hence `min_major_km`.
    #
    # `ARC_IMG` is confounded with size: 41% of *all* craters have `ARC_IMG == 1.0` (small, simple
    # bowl craters are trivially fully traced) vs. just 2.5% of craters >=20km major axis (large
    # craters are more often complex, overlapped by later impacts, or degraded) -- so applying an
    # `ARC_IMG` floor across the whole unfiltered database would just re-select small craters, not
    # better ones. It's a meaningful quality signal only *within* a size band, i.e. combined with
    # `min_major_km` as done here, not as a stand-alone filter (this database has no dedicated
    # degradation-state/"sharpness" grade field, so `ARC_IMG` is the closest proxy available).
    #
