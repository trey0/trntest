...
def test_wac_emp_tile_id_for_bbox_resolves_known_northern_tile():
    ...
    dst_bbox_m = (-50000.0, -50000.0, 50000.0, 50000.0)
    tile_id = lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 135.0, 30.0, MOON_RADIUS_M)
...
def test_wac_emp_tile_id_for_bbox_honors_wavelength_and_ppd():
    dst_bbox_m = (-50000.0, -50000.0, 50000.0, 50000.0)
    tile_id = lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 45.0, 30.0, MOON_RADIUS_M, wavelength_nm=321, ppd=64)
...
def test_wac_emp_tile_id_for_bbox_rejects_unknown_wavelength():
    ...
    with pytest.raises(ValueError, match="wavelength_nm"):
        lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 135.0, 30.0, MOON_RADIUS_M, wavelength_nm=500)


def test_wac_emp_tile_id_for_bbox_raises_beyond_max_latitude():
    dst_bbox_m = (-50000.0, -50000.0, 50000.0, 50000.0)
    with pytest.raises(ValueError, match="beyond WAC_EMP"):
        lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 135.0, 85.0, MOON_RADIUS_M)
...
def test_wac_emp_tile_id_for_bbox_raises_when_straddling_equator():
    ...
    with pytest.raises(ValueError, match="straddles the equator"):
        lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 135.0, 0.0, MOON_RADIUS_M)
...
def test_wac_emp_tile_id_for_bbox_raises_when_straddling_lon_zone_boundary():
    ...
    with pytest.raises(ValueError, match="straddles a WAC_EMP tile"):
        lunaserv.wac_emp_tile_id_for_bbox(dst_bbox_m, 90.0, 30.0, MOON_RADIUS_M)
...
def _write_wac_emp_style_tif(path, reflectance_value, bbox_m, width, height, moon_radius_m):
    ...
    `_write_astropedia_style_tif`), but float32 reflectance values (no int16 planetocentric-radius
    convention) and no `nodata` set (WAC_EMP's own real missing-data sentinel is a set of specific
    non-finite float32 bit patterns this project doesn't need to special-case for a simple, fully-
    valid synthetic AOI)."""
...
def test_reproject_wac_emp_reflectance_to_local_grid_preserves_constant_field(tmp_path):
    # Mirrors test_reproject_astropedia_elevation_to_local_grid_preserves_constant_field, but for a
    # WAC_EMP-style source (real reflectance values, no radius-to-elevation conversion applicable).
    # Native fixture is deliberately much larger than the 10km destination AOI (unlike a real WAC_EMP
    # tile, which spans a whole 60x90-deg quadrant against a typical few-hundred-km footprint AOI, this
    # test's own native/dst size ratio must still be generous enough that the read window's own
    # fractional-source-pixel rounding at its edge -- `window_from_bounds`'s float window vs. `src.read`'s
    # integer-rounded actual read shape, the same mechanism `reproject_astropedia_elevation_to_local_grid`
    # relies on -- doesn't itself clip into the destination AOI's real coverage.
    moon_radius_m = 1_737_400.0
    ...
    result_path = lunaserv.reproject_wac_emp_reflectance_to_local_grid(
...
def test_stretch_reflectance_to_uint8_maps_range_endpoints():
    reflectance = np.array([[lunaserv.DISPLAY_STRETCH_REFLECTANCE_MIN, lunaserv.DISPLAY_STRETCH_REFLECTANCE_MAX]])
    result = lunaserv.stretch_reflectance_to_uint8(reflectance)
    assert result[0, 0] == 0
...
def test_ortho_shaded_filename_no_hapke_ignores_other_flags():
    ...
    assert lunaserv.ortho_shaded_filename(False) == "ortho_shaded_wacemp.tif"
...
def test_ortho_shaded_filename_matches_todays_defaults():
    ...
    assert lunaserv.ortho_shaded_filename(True) == "ortho_shaded_hapke_atc_realparams_normaltilt_wacemp.tif"
...
def test_ortho_shaded_filename_lunaserv_wms_matches_pre_migration_name():
    ...
    assert lunaserv.ortho_shaded_filename(False, ortho_source="lunaserv_wms") == "ortho_shaded.tif"
    assert lunaserv.ortho_shaded_filename(True, ortho_source="lunaserv_wms") == (
...
def test_ortho_shaded_filename_real_params_false_matches_pre_phase_69():
    ...
    assert lunaserv.ortho_shaded_filename(True, real_hapke_params=False) == (
