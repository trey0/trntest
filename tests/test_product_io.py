...
def test_atomic_publish_path_preserves_dest_suffix_on_the_temp_path(tmp_path):
    ...
    with product_io.atomic_publish_path(dest) as tmp:
...
def test_atomic_publish_path_cleans_up_on_exception_even_if_never_created(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="subprocess failed"):
        with product_io.atomic_publish_path(dest):
...
def test_atomic_publish_prefix_writes_via_the_tools_own_suffix_then_renames_to_dest(tmp_path):
    ...
    with product_io.atomic_publish_prefix(dest, "-tile-0.tif") as tmp_prefix:
...
def test_atomic_publish_prefix_leaves_no_temp_output_after_success(tmp_path):
    ...
    with product_io.atomic_publish_prefix(dest, "-cam.tif") as tmp_prefix:
...
def test_atomic_publish_prefix_cleans_up_on_exception_even_if_tool_never_wrote_anything(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="subprocess failed"):
        with product_io.atomic_publish_prefix(dest, "-tile-0.tif"):
            raise RuntimeError("subprocess failed")
...
def test_writes_product_registers_and_returns_function_unchanged():
    ...
    decorated = product_io.writes_product("test_label_a")(my_writer)

    assert decorated is my_writer
    assert decorated() == "wrote"
    assert product_io.writer_of("test_label_a") is my_writer
...
def test_writes_product_raises_on_duplicate_label():
    ...
    product_io.writes_product("test_label_b")(first)

    with pytest.raises(product_io.ProductRegistryError, match="test_label_b"):
        product_io.writes_product("test_label_b")(second)

    # The original registration is untouched by the failed second attempt.
    assert product_io.writer_of("test_label_b") is first


def test_writer_of_returns_none_for_unregistered_label():
    assert product_io.writer_of("test_label_never_registered") is None
...
def test_reads_product_accepts_multiple_registrants():
    ...
    product_io.reads_product("test_label_c")(reader_one)
    product_io.reads_product("test_label_c")(reader_two)

    assert product_io.readers_of("test_label_c") == [reader_one, reader_two]
...
def test_reads_product_returns_function_unchanged():
    ...
    decorated = product_io.reads_product("test_label_d")(reader)
...
def test_readers_of_returns_empty_list_for_unregistered_label():
    assert product_io.readers_of("test_label_never_registered") == []
...
def test_deletes_product_accepts_multiple_registrants():
    ...
    product_io.deletes_product("test_label_e")(deleter_one)
    product_io.deletes_product("test_label_e")(deleter_two)

    assert product_io.deleters_of("test_label_e") == [deleter_one, deleter_two]
...
def test_deletes_product_returns_function_unchanged():
    ...
    decorated = product_io.deletes_product("test_label_f")(deleter)
...
def test_deleters_of_returns_empty_list_for_unregistered_label():
    assert product_io.deleters_of("test_label_never_registered") == []
...
def test_real_pipeline_writers_are_registered():
    ...
    assert product_io.writer_of("dem_filled") is dem_ortho.fetch_dem
    assert product_io.writer_of("ortho_shaded") is dem_ortho.fetch_and_shade_ortho
...
def test_decorator_syntax_works_end_to_end():
    @product_io.writes_product("test_label_g")
    def writer():
        return "written"

    @product_io.reads_product("test_label_g")
    def reader():
        return "read"

    assert writer() == "written"
    assert reader() == "read"
    assert product_io.writer_of("test_label_g") is writer
    assert product_io.readers_of("test_label_g") == [reader]
