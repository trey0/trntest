...
**Read `README.md` first** for current status and a fast, scannable map of what the system does and
how `src/trntest/`/`notebooks/` are organized, kept up to date as work progresses (not just this
file). Then, as needed:
...
- `docs/collaboration.md` — how a session should collaborate with the user on this repo: commit/
  merge review timing, what to do with valuable ad hoc exploration, how to present findings, logging
  non-urgent findings, and recommending merged-branch cleanup at session closeout.
...
- Keep `README.md`'s status, source-files, and notebooks tables current as things change, and record
  ...
  same way past work is recorded there — keep `README.md`/`data-sources.md` themselves scoped
...
- **Notebooks are jupytext-paired and both halves are committed.** Notably:
  ...
  pipeline for one real WAC product — see `README.md`'s Notebooks table). For each, the
  ...
  underlying code changes elsewhere — re-verify a specific claim against `README.md` (or by
