...
| `trn_dataset.py` | `TrnTestDataSet`/`TrnTestEntry`/`TrnTestProduct` — a structured, self-contained dataset folder (`manifest.csv` + `crop`/`hillshade`/`reproject`/`reports` subfolders) replacing `dataset.generate_dataset()`'s flat, all-at-once output layout for the notebook-facing generation path. `TrnTestEntry` holds one manifest row's shared, `functools.cached_property`-cached state (camera, frame timing, the real WAC pipeline's stitched/cropped cubes, DEM/ortho). `TrnTestProduct` (abstract) is the task-queue-facing contract shared by every product type (`raster_path`/`sidecar_json_path`/`generator_name`/`exists()`/`generate()`); `TrnTestImage(TrnTestProduct)` adds the plot-comparison geometry `TrnTestCropImage`/`TrnTestHillshadeImage`/`TrnTestReprojectImage` need, `TrnTestReport(TrnTestProduct)` (below) doesn't and subclasses `TrnTestProduct` directly. `TrnTestDataSet.populate()` drives `tasks.py`'s `huey` queue sequentially, in-process -- no longer safe to run from more than one process concurrently against the same dataset folder (the old cross-process claim-file safety is gone; see `trn_dataset.py`'s own module docstring). `populate_via_workers(product_types, retry_failed, limit, workers)` is `populate()`'s real multi-worker equivalent -- same signature/semantics, but enqueues into `tasks.huey_parallel` and manages its own `huey_consumer -k process` subprocess for the call's duration, so `image.generate()` actually runs across `workers` separate OS processes; `status(huey_instance=tasks.huey_parallel)` sees its failures, a plain `status()` doesn't (independent queues). Since `tasks.py`'s entry-level task granularity (`docs/history.md`'s dated entry), `workers` parallelizes across *entries* only -- every requested, still-pending product type for one entry always runs together, sequentially, in the same task/process, never split across two workers. `task_state()`'s stored-huey-result fallback is therefore also entry-level now: it still reports each product type correctly (`image.exists()` is checked first), just with coarser `failed`-attribution if more than one product type in the same entry's task didn't complete. `TrnTestReprojectImage(TrnTestHillshadeImage)` -- `sat_sim` fed by the real WAC crop's own reflectance (`isis_wac.run_cam2map_for_crop`) instead of the Lunaserv/Astropedia basemap, through the exact same `Camera` `hillshade` renders with (byte-identical `(fu,fv,cu,cv)`, deliberate, for future pixel-grid-identical SSIM/diff comparison -- see `camera.solve_corrected_fov`'s docstring); only `raster_path`/`sidecar_json_path`/`render_label`/`_generate_impl` are overridden, everything else (including `_mapprojected_path`) is inherited via dynamic dispatch. Implemented but deliberately kept out of `PRODUCT_TYPES` -- opt-in only (`product_types=(..., "reproject")`) until wired into a notebook and validated at dataset scale. See `docs/reproject-fov-investigation.md` for `reproject`'s own history. `TrnTestReport(TrnTestProduct)` -- the per-entry HTML report (`reports/<edr_product>/report.html` + `.ipynb`, via `trntest.report.generate_report`), `"report"` in `PRODUCT_TYPES` (default-on, unlike `reproject`) -- self-ensures its own dependency (`entry.hillshade.generate()`) rather than relying on caller-supplied `product_types` ordering, same pattern as `TrnTestReprojectImage`'s dependency on `entry.crop_result`. `TrnTestDataSet.write_index(product_types)` writes `<folder>/status.csv` and `<folder>/reports/index.html` (a nav bar across every entry's report, plus `report.problem_flags`' heuristic flags) -- a separate dataset-wide step (not per-entry, not per-worker, to avoid a shared-file write race under `populate_via_workers()`), run automatically at the end of `populate()`/`populate_via_workers()` via their own `write_index: bool = True` parameter. See `docs/proposed-tasks/report-plan.md`. |
...
- **Open (2026-09-02): per-image Jupyter/HTML reports -- mechanism and dataset wiring done, report
  content still minimal.** `report` is now a fourth `TrnTestProduct` (`TrnTestReport`, `entry.report`,
  `"report"` in `PRODUCT_TYPES`, default-on) alongside `crop`/`hillshade`/`reproject` -- generated
  automatically by `populate()`/`populate_via_workers()` like any other product type, including
  backfilling entries that were already populated before this existed. `TrnTestDataSet.write_index()`
  (also automatic, via `write_index: bool = True`) writes a dataset-wide `status.csv` and
  `reports/index.html` nav bar with cheap heuristic problem flags (`report.problem_flags`, currently
  just low sun elevation). Still open: growing the report's own content to match
  `image_generation.py`'s Phase 5/6/8 comparisons, and richer problem flags (crater sharpness, a
  real tie-point residual) once each has a cheap-enough per-entry path. See
  `docs/proposed-tasks/report-plan.md` for the full design.
