...
- **Open (2026-08-22, Phase 71) — the Phase 70 normal-tilt fix is now wired in as the default, opt-out (`normal_tilt_correction`, default `True`), and independently validated against real ISIS `campt` ground truth at the ellipsoid limit (max |diff| ~0.018 deg) — but the regression it causes against the real WAC crop is still unresolved.** `lunaserv._terrain_photometric_angles`'s DEM-gradient surface normal now uses `dem + sphere_sag` (not raw `dem`) as its gradient input by default, closing the tangent-point-fixed-frame gap Phase 70 found — a deliberate departure from this project's usual "defaults track a confirmed real-image improvement" convention, since the geometry itself is now confirmed correct even though it still makes the brightness-matched diff against the real WAC crop worse (6.89 → 7.58 mean|diff|, Phase 70, unexplained). `normal_tilt_correction=False` remains only as a temporary fallback, expected to be deleted once the regression is understood or the fix is fully trusted. `tests/test_lunaserv_campt_validation.py` (new, permanent `@pytest.mark.heavy` test, `usgscsm` now in `docker/Dockerfile`) is the concrete validation; a follow-up diagnostic (not kept) confirmed `campt`'s own plain angles stay ellipsoid-normal-based even with a DEM shape model attached (`Incidence` byte-identical either way), so it can't validate the real DEM-aware case the way `phocube`'s own broken `localincidence` flag (Phase 70) was supposed to — that gap remains genuinely open. Full rationale in `_terrain_photometric_angles`'s own docstring and `docs/history.md`'s Phase 70/71 entries.
