...
| `render.py` | Runs `sat_sim`/`cam_gen` to produce the rendered `.tif` + CSM/ISD JSON sidecar, then `_correct_csm_focal_length_anisotropy` fixes the sidecar up in place: `cam_gen`'s Pinhole -> CSM Frame conversion collapses an asymmetric `fu`/`fv` (`camera.solve_corrected_fov`) into one isotropic `m_focalLength` (confirmed live: exactly `(fu+fv)/2`), silently losing the real per-axis difference and giving a measurably wrong, near-square `mapproject` footprint (confirmed live: a real Phase 5B blink-overlay misalignment) -- but the CSM Frame model itself has no such limitation (confirmed via `ale`'s own real-instrument formatters, which populate the same `m_iTransL`/`m_iTransS`/`m_transX`/`m_transY` fields anisotropically from real NAIF IK keywords), so this rescales those fields to restore the missing anisotropy rather than abandoning CSM -- live-validated across 4 candidates, `mapproject -t csm` now matches a from-the-`.tsai`-directly `-t pinhole` reprojection to within ~0.3%. `run_mapproject_image` stayed generalized (`camera_path`/`camera_type` rather than hardcoded to CSM) as a result, even though its one live caller (`trn_dataset.TrnTestHillshadeImage._mapprojected_path`) went back to the (now-correct) CSM sidecar. `run_mapproject` (the `RenderResult`-based CSM-sidecar wrapper) has no live caller, kept as-is. See `docs/reproject-fov-investigation.md`. |
...
- **Resolved (on `feature/reproject` branch, not yet merged to `main`): `reproject` implemented.**
  ...
  `main` (notebook wiring, more validation at dataset scale). **Also open, mid-investigation when the
  session ended (token budget)**: a small but real, unexplained residual (~1-8px, <0.6% of the
  footprint, constant not growing with distance from center) between the CSM and Pinhole `mapproject`
  reprojections of the corrected (asymmetric-FOV) camera -- confirmed real via a deterministic-control
  test and a precise ground-point placement check, and confirmed specific to the asymmetric case (a
  symmetric `fu=fv` camera matches exactly, 0px). Root cause not found (would need `usgscsm` source,
  not available in this Docker image) -- see `docs/reproject-fov-investigation.md`'s "OPEN: an
  unexplained small residual" section for exact repro numbers before continuing.
