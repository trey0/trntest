...
- **Resolved: `TrnTestDataSet`/`TrnTestEntry`/`TrnTestImage` implemented** — see `trn_dataset.py`'s
  ...
  pseudocode: `TrnTestImage.plot_overlay()` calls `plotting.plot_overlay_toggle` (the clickable
  on/off toggle), not the plain `plotting.plot_overlay` the design doc names, since the notebook had
  already adopted the toggle version and reverting it would be a real UX regression, not a neutral
  relocation. `reproject` remains unimplemented (reserved folder only), as designed.
