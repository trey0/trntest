...
- **Open, found during the Phase 68 sagitta audit, not investigated**: `lunaserv._terrain_photometric_angles`'s DEM-gradient surface normal (feeds incidence, and part of emission) is computed entirely in the tangent point's own fixed (East, North, Up) frame — never rotated to account for a pixel's own true local vertical tilting away from the tangent point's as the sphere curves (comparable in magnitude to the sagitta effect Phase 68 fixed, ~1-2 deg at a real footprint's edges). Would need a per-pixel rotated local frame, not a single Z-coordinate correction like the sagitta fix — a real, separate piece of work, not yet validated at all.
