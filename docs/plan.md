...
| `product_registry.py` | Intermediate-product access-discipline primitives (`docs/intermediate-product-discipline.md`/`docs/intermediate-product-plan.md`): `writes_product`/`reads_product`/`deletes_product` decorators (a lightweight registry, `writes_product` raising at decoration time on a duplicate label) and `atomic_publish`/`atomic_publish_path` (write-to-uniquely-named-temp-then-atomic-rename, generalizing `cache.cached_get`'s own pattern from fetched files to generated ones — `atomic_publish_path` additionally preserves the destination's real file suffix on the temp path, confirmed live to matter: a generic `.tmp` suffix made real ISIS `to=` calls silently write nothing at all). Applied so far to `lunaserv.fetch_dem`/`fetch_and_shade_ortho`, `isis_wac.run_framestitch`/`crop_for_camera`/`run_cam2map_for_crop`, `render.run_sat_sim` — see `docs/history.md`'s dated entry for the phased rollout and what's still open (the DEM output's own filename still isn't footprint-aware, so two `fetch_dem` calls with genuinely different footprints against the same output dir can still silently disagree about "the" DEM; `TrnTestImage`'s *published* `crop`/`hillshade`/`reproject` outputs aren't registry-covered yet either). |
