...
- DEM: USGS Astropedia GLD100 (`lunaserv.fetch_dem_astropedia`). See
...
- Ortho: WAC_EMP PDS4 reflectance (I/F), normalized to a fixed reference geometry (incidence=30°,
  emission=0°) rather than any particular render's real geometry (`lunaserv.fetch_wac_emp_reflectance`),
...
1. Both sources reprojected onto a shared, camera-centered local Orthographic CRS
   (`lunaserv.fetch_dem_and_ortho`).
2. Ortho despeckled and **relit for this render's real sun/viewing geometry** — a Hapke BRDF via
   ISIS `photomet` by default (`lunaserv.hapke_shade_ortho`), needed because the WAC_EMP source
